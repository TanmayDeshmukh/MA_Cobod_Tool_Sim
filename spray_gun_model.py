...
class SprayGunModel:
    def __init__(self, beta1=1.2, beta2=2.5, a=0.3, b=0.2, f_max=0.0001):
        self.beta1 = beta1
        self.beta2 = beta2
        self.a = a
        self.b = b
    ...
    def get_deposition_canvas(self, orientation: float) -> object:
        ...
        plot_arr_x = np.arange(-rotated_ab[0]-self.viz_resolution, rotated_ab[0]+self.viz_resolution, self.viz_resolution)
        plot_arr_y = np.arange(-rotated_ab[1]-self.viz_resolution, rotated_ab[1]+self.viz_resolution, self.viz_resolution)
    ...
    def visualize_deposition(self, template, X_grid, Y_grid):
        ...
        ax.plot_surface(X_grid, Y_grid, template,
                               antialiased=False, cmap="coolwarm", lw=0.5, rstride=1, cstride=1, alpha=0.5)
        ax.contour(X_grid, Y_grid, template, 10, lw=3, colors="k", linestyles="solid")
        # ax.contour(X_grid, Y_grid, template, zdir='z', offset=self.f_max*1.5, cmap="coolwarm")
        ax.contour(X_grid, Y_grid, template, zdir='x', offset=-self.b*1.2, cmap="coolwarm")
        ax.contour(X_grid, Y_grid, template, zdir='y', offset=self.a*1.2, cmap="coolwarm")
...
if __name__ == '__main__':
    ...
    canvas, X_grid, Y_grid = gun_model.get_deposition_canvas(np.radians(90))
