...
standoff_dist = 0.2
vert_dist_threshold = 0.01
adjacent_tool_pose_angle_threshold = 0.08
vert_iter = 0
...
for section_iter, section_path_group in enumerate(d3sections):
    ...
    # Order subpaths first
    for i in range(len(all_verts)):
        for j in range(i):
            if ((not direction_flag) and all_verts[j][0][2] > all_verts[j+1][-1][2]) or all_verts[j+1][0][2] > all_verts[j][-1][2]:
                print('flipping', all_verts[j], all_verts[j+1])
                all_verts[j], all_verts[j+1] = all_verts[j+1], all_verts[j]
    # print('all_verts', len(all_verts), len(all_normals))
...
xx, yy = np.meshgrid(np.arange(original_mesh.bounds[0][0], original_mesh.bounds[1][0], 0.2), np.arange(original_mesh.bounds[0][1], original_mesh.bounds[1][1], 0.2))
